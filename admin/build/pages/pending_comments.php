...
$id = $_GET['post_id'] ?? 0;
$conn = connect();
$comments = getCommentsByPostId($conn, $id, 'pending');
          ...
          <div class="flex-none w-full max-w-full px-3">
            <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border">
              <div class="p-6 pb-0 mb-0 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
                <h6 class="m-3">pending comments</h6>
                 <a href="posts.php" class="inline-block px-6 py-2 mb-0 ml-4 text-xs font-bold text-center text-white uppercase align-middle transition-all ease-in bg-purple-600 border-0 rounded-lg shadow-md bg-150 leading-pro hover:scale-102 hover:shadow-xs mb-6">Back to Posts</a>
              </div>
              ...
              <?php foreach ($comments as $c): ?>
                ...
                style="margin-left:<?= $c['parent_id'] ? '60px' : '0' ?>;">
                
                <div class="flex items-start gap-4 pl-12">
                    
                    <div class="w-12 h-12 bg-gradient-to-br from-purple-600 to-pink-600
...
<div class="flex items-center gap-2 mb-4">
    <form method="POST" action="approve_comment.php?id=<?= (int)$c['id'] ?>&post_id=<?= (int)$id ?>">
        <input type="hidden" name="comment_id" value="<?= (int)$c['id'] ?>">
        <button type="submit" class="px-3 py-1.5 bg-green-600 text-white rounded-md text-sm shadow-sm hover:bg-green-700 transition">Approve</button>
    </form>

    <form method="POST" action="delete_comment.php?id=<?= (int)$c['id'] ?>&post_id=<?= (int)$id ?>">
        <input type="hidden" name="comment_id" value="<?= (int)$c['id'] ?>">
        <button type="submit" class="px-3 py-1.5 bg-red-600 text-white rounded-md text-sm shadow-sm hover:bg-red-700 transition">Delete</button>
    </form>
</div>
